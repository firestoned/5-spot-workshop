...
live_kind() {
  ...
  if bash workshop/5spot-ctf-capd/setup-background.sh; then
    ...
    # Poll like Flag 1/3, and RE-APPLY each round: on macOS/colima the workload
    # API (reached via a 127.0.0.1 port-forward) can transiently time out, so the
    # first apply may not land. Re-applying self-heals; it's idempotent.
    deadline=$((SECONDS+300))
    until bash workshop/5spot-ctf-capd/step2-taint/verify.sh >/dev/null 2>&1; do
      [ $SECONDS -gt $deadline ] && break
      kubectl --kubeconfig "$HOME/dev-cluster.kubeconfig" apply -f workshop/5spot-ctf-capd/assets/spot-workload.yaml >/dev/null 2>&1 || true
      sleep 15
    done
